...
const typed = new Typed(".typing", {
  strings: ["Web Developer", "Designer","Admin","Application Developer"],
  typeSpeed: 75,
  backSpeed: 60,
  loop: true,
});
...
for (let i = 0; i < totalSidebarList; i++) {
  ...
  a.addEventListener("click", function () {
    document.title = `𝓝𝓰𝓾𝔂𝓮𝓷 𝓣𝓪𝓷 𝓣𝓱𝓪𝓷𝓱 𝓘𝓷4 -  ${a.innerText}`;
    removeBackSection();
...
window.addEventListener("load", function () {
  ...
  if (urlArr.includes(url[1]) && url.length == 2) {
    ...
    document.title = `𝓝𝓰𝓾𝔂𝓮𝓷 𝓣𝓪𝓷 𝓣𝓱𝓪𝓷𝓱 𝓘𝓷4 - ${title}`;
  } else {
    ...
    document.title = `𝓝𝓰𝓾𝔂𝓮𝓷 𝓣𝓪𝓷 𝓣𝓱𝓪𝓷𝓱 𝓘𝓷4 - Home`;
  }
});
